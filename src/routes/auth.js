...
// Note: CSRF protection is now applied globally in server.js, so this endpoint
// doesn't need to apply it again. Just return the token generated by the global middleware.
router.get("/csrf-token", (req, res) => {
  logger.info("CSRF token requested");
  
  try {
    const token = req.csrfToken();
    logger.debug('Generated CSRF token:', { token: token.substring(0, 20) + '...' });
